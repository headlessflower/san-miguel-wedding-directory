    ...
    <div class="container">
      <div class="appFooter__panel">
        <div class="appFooter__brandCol">
          <NuxtLink :to="home" class="appFooter__brand" :aria-label="t('brand.name')">
            <span class="appFooter__mark">SM</span>
            <span class="appFooter__brandName">{{ t("brand.name") }}</span>
          </NuxtLink>

          <p class="appFooter__summary">
            {{
              isEs
                ? "Directorio bilingue para parejas que planean bodas en San Miguel de Allende y para proveedores que quieren llegar a ellas."
                : "A bilingual directory for couples planning San Miguel de Allende weddings and the vendors ready to serve them."
            }}
          </p>
        ...
        <div class="appFooter__bottom">
          <p>{{ isEs ? "Todos los derechos reservados." : "All rights reserved." }} © {{ year }}</p>
          <NuxtLink :to="newsletter" class="appFooter__newsletter">
            {{ isEs ? "Recibir ideas para bodas" : "Get wedding planning ideas" }}
          </NuxtLink>
        </div>
      </div>
    </div>
...
const linkGroups = computed(() => [
  ...
  },
  {
    ...
    links: [
      ...
      { label: isEs.value ? "Newsletter de bodas" : "Wedding newsletter", to: newsletter.value },
      { label: isEs.value ? "Guia de lugares" : "Venue guide", to: venues.value },
    ],
  },
  {
    ...
    links: [
      ...
      { label: isEs.value ? "Por que anunciarte" : "Why advertise", to: whyAdvertise.value },
      { label: isEs.value ? "Planes y precios" : "Plans and pricing", to: advertise.value },
      { label: isEs.value ? "Solicitar informacion" : "Request information", to: `${advertise.value}#lead` },
    ],
  },
]);
</script>
...
.appFooter__panel {
  ...
  border-radius: clamp(1.75rem, 4vw, 3.75rem);
